...
setup_datalist = ->
  ...
  yamoda.entry.fetch_2D_preview_images($("#data_list_table td.2d-preview"), "100%")
  $("#data_list_table_wrapper .datatables-bottom-right-bar").append($("#data_action").remove())
  yamoda.utils.setup_column_filter_boxes($("#data_list_table th.footer input.filter-input"), dtable)
  yamoda.utils.setup_datatable_selection($table)
  $('a[rel=tooltip]').tooltip()

  get_selected_data_rows = ->
    $("#data_list_table>tbody>tr.row-selected")
